...
`puzzle_01` through `puzzle_20` are original workshop exercises. They use the
...
the cited chapter. Names preserve the source locator.
...
| `smullyan_32` | 32 | All-knaves and exactly-one-knave claims. |
| `smullyan_33` | 33 | A’s conjunction about A and B. |
| `smullyan_34` | 34 | A’s and B’s same-kind statements. |
...
Staff-only witnesses and no-model checks are in `DROP` regions. Student proof
bodies use `TO_SORRY` and `SORRY_END`.
