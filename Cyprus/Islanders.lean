...
/-!
Support code for the day 2 knights-and-knaves puzzles.
...
`open Cyprus.Islanders` activates the `verdict` and `claim%` syntax.
-/

namespace Cyprus.Islanders
...
scoped syntax "verdict" "(" islanderClaim,+ ")" : term
...
scoped syntax "claim% " term:max term:max : term
